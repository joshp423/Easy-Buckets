...
export default function VideoPlayer({ videoUrl, ref }: videoPlayerProps) {
  ...
  const onReady = (event: YouTubeEvent) => { //once player has loaded(event) playerRef = ytplayer object
    playerRef.current = event.target;
  };

  useImperativeHandle(ref, () => ({//first argument is the ref passed from parent, second is function returning what ref.current should become (getCurrentTimestamp)
    getCurrentTimestamp: () => {
